...
CHECK_KEYS: 	CMP		R5, #73
    			BLEQ	KEY_ISR				// pass R0 as a parameter to KEY_ISR
...
KEY_ISR:		PUSH {R0-R12, LR}
				...
				BEQ SETRUN             //Branch to what key does

				CMP R7, #0x02       //If R7 (key value) is key1
				BEQ SPEEDUP             //Branch to what key does

				CMP R7, #0x04       //If R7 (key value) is key2
				BEQ SPEEDDOWN             //Branch to what key does
...
SPEEDUP:	PUSH {R0-R10}
            LDR R1, =0xFF202000         //setup base address of timer

			//Retrieve current delay from reg
			LDR R10, [R1, #0x8] 		//lower bits

			LDR R9, [R1, #0xC]			//upper bits
			LSL R9, #16					//shift to align
			ORR R10, R9					//merge

			LSR R10, #1					//half the delay value

			//Put back the new delay value
            STR R10, [R1, #0x8]			//restore lower bits
			LSR R10, #16
            STR R10, [R1, #0xC]			//restore upper bits
               
            MOV R0, #0b101              //setup timer, STOP=0, START=1, CONT=0, ITO=1
            STR R0, [R1, #0x04]         //store values into timer reg

			POP {R0-R10}

            B       RETURN

SPEEDDOWN:	PUSH {R0-R10}
            LDR R1, =0xFF202000         //setup base address of timer
			...
			MOV R8, #2
			MUL R10, R8					//double the delay value
...
TIMER_ISR:  PUSH {R0-R5}
            ...
            ADD R5, R4
            STR R5, [R3]                //store run + count into count
...
CONFIG_TIMER:   
                //Set timer to 25000000 for 0.25 delay   (17D7840 in HEX)                       
                PUSH {R0-R5}
                LDR R1, =0xFF202000         //setup base address of timer
                
                //Store number in timer
                MOVW R2, #0x7840             //bottom HEX
                STR R2, [R1, #0x8]
                MOVW R2, #0x17D             //top HEX
                STR R2, [R1, #0xC]
...
CONFIG_KEYS:                                    
				...
				LDR	R1, =0xFF200050		// pushbutton KEY base address
				MOV	R2, #0xF			// set interrupt mask bits
				STR	R2, [R1, #0x8]		// interrupt mask register is (base + 8)
